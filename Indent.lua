...
if not HackIndent.revision or revision > HackIndent.revision then
...
-- default color scheme of FAIAP
   ...
   defaultColorTable[0] = "|r"
	...
	local function set(color, ...)
		for i=1,select('#',...) do
			hackColorTable[select(i,...)] = color
		end
	end
	set('|c008DBBD7', tokens.TOKEN_KEYWORD)
	set('|c00FFA600', tokens.TOKEN_SPECIAL, '..','...')
	set('|c00c27272', tokens.TOKEN_STRING, tokens.TOKEN_NUMBER)
	set('|c00888855', tokens.TOKEN_COMMENT_SHORT, tokens.TOKEN_COMMENT_LONG)
	set('|c00ccbbaa', '{','}','[',']','(',')','+','-','/','*')
